...
public class ServoPWMControl{
    ...
    public ServoPWMControl(Servo servo){
        ...
        try {
            ...
            getServoPWMStatus = LynxServoController.class.getDeclaredMethod("internalGetPwmEnable", int.class);
            getServoPWMStatus.setAccessible(true); // 绕过权限限制
    ...
    public String getStatus(){
        try {
            return (String) getServoPWMStatus.invoke(controller, ServoPort);
        } catch (Exception e) {
            throw new RuntimeException("TM的调用方法炸了!");
        }
    }
